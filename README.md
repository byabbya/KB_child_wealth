...
KB스타뱅킹 내부의 자녀 자산관리 메뉴를 가정해 구현했습니다. 현재는 고정된 샘플 자녀와 자산 데이터를 사용하며, 실제 계좌 조회·상품 가입·거래는 지원하지 않습니다.
...
테스트는 선호 순위에 따른 자산배분, 투자기간 안전장치, KB 상품 필터, 증여세와 투자비용 계산, LLM 도구 호출, 정책 위반 보정과 연결 실패 시 전환을 확인합니다.

## 제한사항

- 실제 자녀 계좌 조회, 법정대리인 확인, 상품 가입, 주문 실행과 세무신고는 구현 범위에 포함하지 않습니다.
- 상품·시장 데이터는 샘플 스냅샷이므로 실제 판매 상태, 금리와 거래 조건은 가입 전에 공식 자료로 다시 확인해야 합니다.
