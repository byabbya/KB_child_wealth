...
### 1️⃣ 자산 현황·장기 운용 성향 분석

- 자녀의 예금·적금과 투자자산을 한눈에 확인합니다.
- 목표금액, 투자기간, 월 저축액과 자산·투자방식 선호 순위를 분석합니다.

### 2️⃣ 맞춤형 포트폴리오 구성

- 현재 자산과 보호자의 투자 성향, 기준일의 시장 자료를 바탕으로 목표 자산 비중과 추천 이유를 제시합니다.
- 금융 조건을 검증한 뒤 적합한 KB국민은행·KB증권 상품과 투자금액을 연결합니다.

### 3️⃣ 지속적인 점검과 리밸런싱

- 지난 포트폴리오의 성과를 확인하고 목표 포트폴리오와 비교합니다.
- 신규 납입금과 만기자금을 활용해 목표 비중에 가까워지는 방법을 제안하고, 목표·기간의 변화에 맞춰 포트폴리오를 다시 점검합니다.

-----

## 🔄 이용 흐름
```
자산 현황 확인 → 목표·성향 설정 → 포트폴리오 분석 → KB 상품 매칭 → 지속 관리
```
...
- 실제 자녀 계좌 조회, 법정대리인 확인, 상품 가입, 주문 실행과 세무신고는 프로토타입 구현 범위에 포함하지 않았습니다.
- 투자성향은 사용자의 선호 순위와 투자기간을 중심으로 반영하며, 손실 감내 수준을 포함한 성향 점수화는 구현하지 않았습니다.
- 목표 포트폴리오는 규칙 기반 기준안을 AI가 제한적으로 조정하는 방식으로 산출하며, 기대수익률·위험·공분산을 활용한 평균-분산 최적화는 구현하지 않았습니다.
- 리밸런싱은 현재 비중과 목표 비중을 비교해 신규 납입금과 만기자금의 배분안을 제시하는 수준이며, 실시간 수익률과 시장 변화를 반영한 자동 점검·조정은 지원하지 않습니다.
