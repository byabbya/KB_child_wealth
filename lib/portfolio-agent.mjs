...
export const COMBINED_ADVICE_SCHEMA = {
  type: "object",
  properties: {
    userAnalysis: USER_ANALYSIS_SCHEMA,
    marketAnalysis: MARKET_ANALYSIS_SCHEMA,
    proposal: ALLOCATION_RESPONSE_SCHEMA,
  },
  required: ["userAnalysis", "marketAnalysis", "proposal"],
};
...
function portfolioAdvicePrompt(input) {
  return JSON.stringify(
    {
      child: input.child,
      profile: input.profile,
      currentPortfolio: input.currentPortfolio,
      marketSnapshot: input.marketFreshness.fresh ? input.marketSnapshot : null,
      ...
      instructions: [
        "userAnalysis에서 자산·투자방식 순위, 목표금액 차이, 현재 자산 집중도와 유동성을 설명",
        "marketAnalysis에서 국내·미국시장과 ETF·개별종목 전망을 positive, neutral, cautious 중 하나로 작성",
        "시장자료가 오래됐으면 marketAnalysis를 모두 neutral, confidence는 low, evidenceIds는 빈 배열로 작성",
        "proposal에는 8개 자산군 비중과 자산군별 근거만 작성",
        "상품 ID, 상품명, 금액, 금리, 세금, 수수료를 작성하지 말 것",
        "시장자료가 중립 또는 오래된 경우 사용자 조건과 기준안을 우선할 것",
...
export async function runPortfolioAdvisor({ provider, input }) {
  ...
  try {
    const combined = await provider.complete(
      [
        { role: "system", content: baseSystemPrompt("사용자·시장·자산배분 통합 분석가") },
        {
          role: "user",
          content: portfolioAdvicePrompt(enrichedInput),
        },
      ],
      COMBINED_ADVICE_SCHEMA,
    );
    const combinedResult = extractJson(combined.content);
    const userAnalysis = sanitizeUserAnalysis(combinedResult.userAnalysis);
    const marketAnalysis = marketFreshness.fresh
      ? sanitizeMarketAnalysis(combinedResult.marketAnalysis, enrichedInput)
      : neutralMarketAnalysis(enrichedInput);
    const analysis = { user: userAnalysis, market: marketAnalysis };
    const originalProposal = combinedResult.proposal;
    ...
    return fallbackResult(enrichedInput, error);
  }
}
