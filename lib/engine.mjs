...
export function generatePlan({
  ...
  proposedGift = /** @type {Record<string, unknown> | null} */ (null),
}) {
...
export function applyAdvisorProposal({
  ...
  const rationaleByClass = new Map(
    (proposal.allocationRationales ?? []).map((item) => [item.assetClass, item.rationale]),
  );
  const taxFeeEngine = new InvestmentTaxFeeEngine(investmentTaxRules, feeAssumptions);
  // AI가 검증된 자산군 비중만 제안한 뒤, 기존 정책을 통과한 KB 후보를 매칭합니다.
  const recommendations = basePlan.recommendations
    .filter((candidate) => Number(target[candidate.assetClass] ?? 0) > 0)
    .map((candidate) => {
      ...
      return {
        ...
        actionAmount: Math.max(
          0,
          targetAmount - basePlan.current.amounts[assetClass],
        ),
        reason: rationaleByClass.get(assetClass) || candidate.reason,
        advisorAction:
          targetWeight > Number(basePlan.current.weights[assetClass] ?? 0) + 1
            ? "추가입금"
            : targetWeight < Number(basePlan.current.weights[assetClass] ?? 0) - 1
              ? "매도검토"
              : "유지",
  ...
  const matchedClasses = new Set(recommendations.map((item) => item.assetClass));
  const unmatchedLimitations = ASSET_CLASSES
    .filter((assetClass) => target[assetClass] > 0 && !matchedClasses.has(assetClass))
    .map((assetClass) => ({
      assetClass,
      message: `${ASSET_LABELS[assetClass]}은 검증된 KB 상품 후보가 없어 매칭하지 못했습니다.`,
    }));
  ...
  return {
    ...
    target,
    recommendations,
    limitations: [...basePlan.limitations, ...unmatchedLimitations],
